...
# T01: Add a dedicated in-memory Key Vault behavior suite and close the slice with sequential regressions

**Slice:** S04 — In-memory Key Vault basics
**Milestone:** M001

## Description

Add focused Key Vault behavior proof outside the DI registration suite so S04 owns explicit evidence instead of relying on the S01 registration tests.
...
- `dotnet test ./Azure.InMemory.sln --filter FullyQualifiedName~InMemoryKeyVaultBehaviorTests`
- `dotnet test ./Azure.InMemory.sln --filter FullyQualifiedName~KeyVault`
- `dotnet test ./Azure.InMemory.sln`

## Observability Impact

- Signals added/changed: named assertions over overwrite/version/validation behavior plus continued use of `SecretExists(...)` and `SecretNames`
- How a future agent inspects this: run the focused Key Vault suite and inspect the assertions in `tests/Azure.InMemory.Tests/KeyVault/InMemory/InMemoryKeyVaultBehaviorTests.cs`
- Failure state exposed: whether the regression is in DI resolution, latest-value replacement, version generation, case-insensitive lookup, or fail-fast validation

## Inputs

- `src/Azure.InMemory/KeyVault/IAzureKeyVaultFactory.cs` — stable Key Vault seam the task must prove without widening
- `src/Azure.InMemory/KeyVault/InMemory/InMemoryKeyVaultFactory.cs` — current in-memory adapter that the focused suite will exercise through DI
- `src/Azure.InMemory/KeyVault/InMemory/InMemoryKeyVaultState.cs` — existing shared state root with secret inspection helpers and fail-fast validation
- `src/Azure.InMemory/DependencyInjection/AzureKeyVaultRegistrationExtensions.cs` — registration path that must continue to resolve the in-memory factory
- `tests/Azure.InMemory.Tests/DependencyInjection/KeyVaultProviderRegistrationTests.cs` — current mixed registration-and-behavior proof to narrow back to DI coverage

## Expected Output

- `tests/Azure.InMemory.Tests/KeyVault/InMemory/InMemoryKeyVaultBehaviorTests.cs` — focused behavior proof for the in-memory Key Vault seam
- `tests/Azure.InMemory.Tests/DependencyInjection/KeyVaultProviderRegistrationTests.cs` — DI-focused registration suite with the slice proof extracted into the dedicated behavior file
